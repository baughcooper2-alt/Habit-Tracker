  ...
  if (habit.type === "BOOLEAN") {
    ...
    return (
      <button
        type="button"
        onClick={onToggle}
        className={`flex w-full items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left shadow-soft transition-all active:scale-[0.98] ${
          done
      ...
      >
        ...
        <span className={`flex-1 text-sm font-bold ${done ? "text-white" : "text-ink"}`}>
          {habit.name}
        </span>
        <span
...
function QuantityCard({
  ...
  return (
    <div className={`rounded-xl border border-transparent ${theme.soft} px-3.5 py-2.5 shadow-soft`}>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            {met && (
              <span className="shrink-0 text-[10px] font-bold text-emerald-400">✓ met</span>
            )}
          </div>
          {editing ? (
            <div className="mt-0.5 flex items-center gap-1.5">
              <input
                ...
                onBlur={commitEdit}
                className="w-20 rounded-lg border border-cocoa/20 bg-black/25 px-2 py-0.5 text-xs font-semibold text-ink outline-none focus:border-ink/40"
              />
              <span className="text-[11px] font-medium text-cocoa">{habit.unit}</span>
            </div>
          ) : (
            <button
              type="button"
              ...
              className="text-[11px] font-semibold text-cocoa underline decoration-dotted underline-offset-2"
            >
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {habit.quickAdds.map((amt) => (
          <button
            ...
            className="rounded-full bg-white/10 px-2.5 py-0.5 text-[11px] font-bold text-ink shadow-sm active:scale-95"
          >
            +{amt}
          </button>
        ))}
        {value > 0 && (
          <button
            ...
            className="rounded-full bg-white/10 px-2.5 py-0.5 text-[11px] font-bold text-cocoa active:scale-95"
          >
